...
public class CallManager implements InterlocutorListener {
    ...
    private void showIncomingCall(final InterlocutorUI ic) {
        ...
        try {
            EventQueue.invokeAndWait(() -> {
                ...
                incomingCall.getAcceptButton().addActionListener((ActionEvent e) -> {
                    SparkManager.getMainWindow().toFront();
                    closeToaster(ic);
                    SoftPhoneManager.getInstance().getDefaultGuiManager().answer();
                });
                incomingCall.getRejectButton().addActionListener((ActionEvent e) -> {
                    closeToaster(ic);
    ...
    public void interlocutorAdded(final InterlocutorUI interlocutorUI) {
        ...
        interlocutorUI.getCall().addStateChangeListener((CallStateEvent evt) -> {
    ...
    /**
     * Retrieves the <code>PhonePanel</code> object from the
     * <code>ChatRoom</code>. If no PhonePanel exists, then null is returned.
     *
    ...
    /**
     * Returns the <code>SparkTab</code> that acts as a container to this
     * PhonePanel.
     *
     * @param phonePanel the <code>PhonePanel</code>
     * @return the SparkTab.
