...
public class PrivacyListTree extends JPanel implements SparkPrivacyListListener {
    ...
    private void createCurrentListInfoPanel() {
        ...
        _actList.addActionListener((ActionEvent e) -> {
            _pManager.declineActiveList();
        });
        _defList = new RolloverButton();
        _defList.addActionListener((ActionEvent e) -> {
     ...
     * jmenuitem
     *
     * @param menu where the method should add the remove option
     * @param node the node what is selected
     */
    private void addMenuForLeaf(JPopupMenu menu, final PrivacyTreeNode node) {
        ...
        menu.add(remUser);
        remUser.addActionListener((ActionEvent e) -> {
    ...
    private void addMenuForGroupNodes(JPopupMenu menu, final PrivacyTreeNode node) {
        ...
        addContact.addActionListener((ActionEvent e) -> {
            ...
            col.stream().map((pI) -> {
                ...
                list.addItem(pI);
                return pI;
            }).map((pI) -> new PrivacyTreeNode(pI)).forEach((newChild) -> {
    ...
    private void addMenuForListNodes(JPopupMenu menu, final PrivacyTreeNode node) {
        ...
        addList.addActionListener((ActionEvent e) -> {
     ...
     *
     * @param node the node that should be added. this is the childnode
     * @param parent the parent node, where the node should be added to
     */
    private void addListNode(PrivacyTreeNode node, DefaultMutableTreeNode parent) {
        ...
        plist.getPrivacyItems().stream().forEach((pI) -> {
    ...
    private void loadPrivacyLists() {
        ...
        _pManager.getPrivacyLists().stream().forEach((list) -> {
