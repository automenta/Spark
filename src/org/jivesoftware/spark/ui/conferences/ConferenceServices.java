...
/**
 * Conference plugin is reponsible for the initial loading of MultiUser Chat
 * support. To disable plugin, you can remove from the plugins.xml file located
 * in the classpath of Communicator.
 */
public class ConferenceServices implements InvitationListener {
    ...
    public ConferenceServices() {
        ...
        if (mucSupported) {
            ...
            actionMenuItem.addActionListener((ActionEvent e) -> {
            ...
            PresenceListener presenceListener = (final Presence presence) -> {
                SwingUtilities.invokeLater(() -> {
                    SparkManager.getChatManager().getChatContainer().getChatRooms().stream().filter((room) -> (room instanceof GroupChatRoom)).map((room) -> {
